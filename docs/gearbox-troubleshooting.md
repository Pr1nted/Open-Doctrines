...
a bomb costs nothing. See [modding.md](modding.md) for the full table.
...
| `WASM module load failed: fast interpreter offset overflow` | The module is too big for WAMR's fast interpreter. | A function exceeds the INT16_MAX operand-stack or local-count limit. Only CPython does this in practice. Build the host with `-DOD_MODS_FAST_INTERP=OFF`; see [gearbox-languages.md](gearbox-languages.md#why-python-needs-a-different-build). |
